...
class ScrollableTabBar extends BaseCommon {
  constructor(props) {
    super(props);
    ...
    this.state._leftTabUnderline.addListener(
      ({ value }) => (this._leftTabUnderlineValue = value),
    );
    this.state._widthTabUnderline.addListener(
      ({ value }) => (this._widthTabUnderlineValue = value),
    );

    this.displayName = 'ScrollableTabBar';
    // this.debug= true;
  }
  componentDidMount() {
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    ...
    if (
      ...
      this.setStateSafe({ _containerWidth: null });
    }
  }
  ...
  updateTabPanel = (position, pageOffset) => {
    ...
    newScrollX -=
      (WINDOW_WIDTH - (1 - pageOffset) * tabWidth - pageOffset * nextTabWidth) /
      2;
  ...
  onTabContainerLayout = e => {
    ...
    this.setStateSafe({ _containerWidth: width });
